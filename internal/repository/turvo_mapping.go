...
// turvoCustomerListEntryToModel maps a Turvo customer list entry to the internal domain model.
func turvoCustomerListEntryToModel(tc turvoCustomerListEntry) model.Customer {
	c := model.Customer{
		ExternalTMSId: fmt.Sprintf("%d", tc.ID),
		Name:          tc.Name,
	}
